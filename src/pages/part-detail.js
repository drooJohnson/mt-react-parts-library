...
class PartDetail extends React.Component {
	...
	details = (part) => {
		...
		description = [( part.description !== '' ? part.description : <NoData key={'noDescription'}>No Description</NoData> )]

		dimensions = part.dimensions

		tolerance = [( part.minimumTolerance !== '' ? '±' + part.minimumTolerance : <NoData key={'noTolerance'}>No Tolerance Value Provided</NoData> ),' ',( part.minimumToleranceUnits !== '' ? part.minimumToleranceUnits : <NoData key={'noToleranceUnits'}>Unknown Units</NoData> )]

		let {grade, type} = part.material
		material = [( type !== '' ? type : <NoData key={'noMaterialType'}>No Type Provided</NoData> ),' ',( grade !== '' ? grade : <NoData key={'noMaterialGrade'}>No Grade Provided</NoData> )]

		machineTypes = [( part.machineTypes[0] !== '' ? part.machineTypes.join(', ') : <NoData key={'noMachineType'}>No Machine Types Provided</NoData> )]
